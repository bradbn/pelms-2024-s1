...
class PelmsTkGuiClass(tk.Tk):
    ...
    def config_frame_menu(self):
        ...
        TEST_MODE - test can be passive i.e. no current sent or active current 
                    signal sent.  Passive mode is for camera adjustment.
        """
        ...
        opt_menu_cam_exp = tk.OptionMenu(self.frame_menu, 
            CAM_EXP_MS, "0", "10", "20", "40", "60", "80", "100", "500", \
            "1000", "5000")
        ...
        opt_menu_num_img_pair = tk.OptionMenu(self.frame_menu, 
            NUM_IMG_PAIR, "1", "2", "5", "8", "10", "20", "50", "80", "100", 
            "120", "150", "180", "200")
        ...
        # Menu Option:      Test Mode
        # Label:            label_keep_img_pair 
        # Workspace Config: N/A
        label_test_mode = tk.Label(self.frame_menu)
        ...
        button_el_measure.configure(text="EL Measure",
            command=lambda: self.el_measure(NUM_IMG_PAIR, KEEP_PAIR_FILES_FLAG, 
                CAM_RES, CAM_EXP_MS, TEST_MODE), height=2, width=20)
    ...
    def el_measure(self, NUM_IMG_PAIR, KEEP_PAIR_FILES_FLAG, CAM_RES, 
                   CAM_EXP_MS, TEST_MODE):
        """ EL Measurement
        Performs the EL MEasuremnt.
        ...
        print("EL Measurement: test_mode: " + test_mode)
        ...
        if cam_exp_ms > 0:
            ...
            picam2.set_controls({"ExposureTime": cam_exp_ms, \
                "AnalogueGain": 1.0})
        ...
        for pair_index in range(num_img_pair):
            ...
            img_drk_RGB_8b = (img_drk_RGB_16b // 256).astype(np.uint8)

            img_PIL_full = Image.fromarray(img_drk_RGB_8b)
            ...
            img_bri_RGB_16b = cv2.cvtColor(img_bri_BAY_16b, 
                cv2.COLOR_BAYER_BG2RGB)
            img_bri_RGB_8b = (img_bri_RGB_16b // 256).astype(np.uint8)
            img_PIL_full = Image.fromarray(img_bri_RGB_8b)
            img_PIL_resize = img_PIL_full.resize((406, 304))            
            self.wks_img = ImageTk.PhotoImage(img_PIL_resize)            
            self.label_wks_img.config(image=self.wks_img)
        ...
        if (test_mode == "ACTIVE") or (test_mode == "ACTIVE-T"):
            ...
            fs_img_dif_GRY_16b = cv2.cvtColor(fs_img_dif_RGB_16b, \
                cv2.COLOR_BGR2GRAY)
            fs_img_dif_GRY_16b_file = "/home/portableel/Desktop/" \
                + img_file_prefix + "_GRY.tif"
            cv2.imwrite(fs_img_dif_GRY_16b_file, fs_img_dif_GRY_16b)


        self.wks_txt = "EL Measurement: FINISH File " + img_file_prefix + ".tif"
        self.label_wks_txt.config(text=self.wks_txt)

        fs_img_dif_RGB_8b = (fs_img_dif_RGB_16b // 256).astype(np.uint8)
        img_PIL_full = Image.fromarray(fs_img_dif_RGB_8b)
        img_PIL_resize = img_PIL_full.resize((406, 304))            
        self.wks_img = ImageTk.PhotoImage(img_PIL_resize)            
        self.label_wks_img.config(image=self.wks_img)
