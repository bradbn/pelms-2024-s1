...
class PelmsTkGuiClass(tk.Tk):
    ...
    def workspace_el_measure_page_setup(self):
        ...
        self.elm_img_pair_count = tk.StringVar(self)#"50"
        self.elm_img_pair_count.set("5")
        self.elm_camera_iso = StringVar(self)
        self.elm_camera_iso.set("800")
        self.elm_camera_res = StringVar(self)
        self.elm_camera_res.set("(2048,2048)")
        ...
        elm_img_pair_count_label = tk.Label(self.frame_workspace, 
            text="Image Pair Count: ")    
        elm_img_pair_count_optmenu = tk.OptionMenu(self.frame_workspace, 
            self.elm_img_pair_count, "1", "2", "5", "10", "20", "50", "80", 
            "100")
        
        elm_camera_iso_label = tk.Label(self.frame_workspace, 
            text="Camera ISO: ")    
        elm_camera_iso_entry = tk.Entry(self.frame_workspace, width=15, 
            textvariable=self.elm_camera_iso, )

        elm_camera_res_label = tk.Label(self.frame_workspace, 
            text="Camera Resolution: ")    
        elm_camera_res_entry = tk.Entry(self.frame_workspace, width=15, 
            textvariable=self.elm_camera_res)




        elm_img_pair_count_label.place(relx=0.05, rely=0.04)  
        elm_img_pair_count_optmenu.place(relx=0.30, rely=0.02)
        
        elm_camera_iso_label.place(relx=0.05, rely=0.12)    
        elm_camera_iso_entry.place(relx=0.30, rely=0.12)

        elm_camera_res_label.place(relx=0.05, rely=0.20)    
        elm_camera_res_entry.place(relx=0.30, rely=0.20)

        elm_status_label.place(relx=0.3, rely=0.9, anchor="w")
        elm_status_val.place(relx=0.4, rely=0.9, anchor="w")

        #elm_update_config_button = tk.Button(self.frame_workspace)
        #elm_update_config_button.configure(text="Update Camera Config")
        #elm_update_config_button.configure(height=3, width=20)
        #elm_update_config_button.place(relx=0.6, rely=0.05)
        ...
        elm_run_button.config(text="Run EL Measurement",
            command=lambda: self.command_run_elm(elm_status_val, 
                self.elm_img_pair_count.get(), self.elm_camera_iso.get(), 
                self.elm_camera_res.get()))
        elm_run_button.config(height=2, width=40)
        elm_run_button.place(relx=0.2, rely=0.3)


            
    def command_run_elm(self, status_val, pair_count, cam_iso, cam_res):

        print("elm_status:         " + self.elm_status)
        print("elm_img_pair_count: " + pair_count)
        print("elm_camera_iso:     " + cam_iso)
        print("elm_camera_res:     " + cam_res)
        ...
        for pair_count_num in range(int(pair_count)):
            print(pair_count_num)
            self.elm_status = "image capture pair: " + str(pair_count_num)
            status_val.configure(text=self.elm_status)
            status_val.update()
            time.sleep(0.25)

            # Get a raw frame
            img_raw_capture = picam2.capture_array("raw").view(np.uint16)

            # Convert to 16-bit
            img_raw_16bit = img_raw_capture * 16

            # Debayer
            img_colour = cv2.cvtColor(img_raw_16bit, cv2.COLOR_BAYER_BG2RGB)

            # Write to file
            cv2.imwrite("C.tif", img_colour)
    ...
    def workspace_file_transfer_setup(self):
        ...
        src_file_img_label.place(relx=0.05,rely=0.55).pack()
    ...
    def cmd_file_transfer_update_path(self, path_type, val_label, img_label):
        ...
        if path_type == 'src_dir':
            ...
            img_label.config(image=src_img).pack(padx=0, pady=1)
